...
class IndependentSetInstance(GraphInstance):
    ...
    def __repr__(self) -> str:
        return (
            f"IndependentSetInstance(name={self.name!r}, "
    ...
    def to_sat(self) -> SATInstance:
        ...
        # C1: every position in the independent set is occupied
        ...
        # C4: if two vertices are connected, they cannot both be in the independent set
        for u in range(1, n + 1):
            for v in range(u + 1, n + 1):
                if self.has_edge(u, v):
                    for i in range(1, k + 1):
